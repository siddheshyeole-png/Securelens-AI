...
const server = app.listen(5097, async () => {
  console.log("==========================================");
  console.log("[SecureLens AI] Real PyTorch Audio Model Integration Test");
  console.log("==========================================");
  ...
  formData.append("media", buffer, { filename: "test_sample_audio.wav", contentType: "audio/wav" });

  try {
    const res = await axios.post("http://localhost:5097/api/analyze", formData, {
    ...
    const isAiReal = typeof res.data.scores.aiGenerated === "number" && res.data.scores.aiGenerated > 0;
    const isDfNull = res.data.scores.deepfake === null;

    if (isSuccess && isAiReal && isDfNull) {
      console.log("\n✅ REAL PYTORCH AUDIO SYNTHETIC MODEL TEST PASSED 100%!");
      process.exit(0);
    } else {
